##
## Copy from mruby-redis Redis Test
##
...
assert("Redis#new") do
  assert_raise(Redis::ConnectionError) { Redis::Retryable.new "10.10.10.10", 6379 }
  assert_raise(Redis::ConnectionError) { Redis::Retryable.new "10.10.10.10", 6379, 0 }
  assert_raise(Redis::ConnectionError) { Redis::Retryable.new "10.10.10.10", 6379, 1 }
  assert_raise(TypeError)    { Redis::Retryable.new "10.10.10.10", 6379, nil }
end
...
assert("Pipelined commands") do
  redis = Redis::Retryable.new HOST, PORT
...
assert("Redis#publish") do
  producer = Redis::Retryable.new HOST, PORT
...
assert("Redis#multi") do
  client1 = Redis::Retryable.new HOST, PORT
  client2 = Redis::Retryable.new HOST, PORT
  client1.del "hoge"
